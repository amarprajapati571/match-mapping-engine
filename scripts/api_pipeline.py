...
def main():
    ...
    # ── Step 1-2: Fetch Bet365 + all enabled providers ──
    fetch_data: dict = {}
    needs_separate_bet365 = any(p.get("paginated", True) for p in providers)
    bet365_path = os.path.join(DATA_DIR, "bet365_raw.json")

    if args.skip_fetch:
        print("Step 1-2: Loading cached data (--skip-fetch)...")
        try:
            with open(bet365_path, "r") as f:
                fetch_data["bet365"] = json.load(f)
        except FileNotFoundError:
            fetch_data["bet365"] = []
        print(f"  Bet365 (shared): {len(fetch_data['bet365'])} matches")
        ...
        for p in providers:
            ...
            try:
                ...
                pass
            b365_info = f" (+{len(fetch_data.get(f'{name}_bet365', []))} B365)" if fetch_data.get(f"{name}_bet365") else ""
            print(f"  {name}: {len(fetch_data[name])} matches{b365_info}")
    else:
        print(f"Step 1-2: Fetching {'Bet365 + ' if needs_separate_bet365 else ''}{len(providers)} providers concurrently...")
            ...
            for future in as_completed(futures):
                ...
                try:
                    ...
                    if is_paginated or label == "bet365":
                        fetch_data[label] = raw
                    else:
                        fetch_data[label] = raw["matches"]
                        if raw.get("bet365"):
                            fetch_data[f"{label}_bet365"] = raw["bet365"]
                            print(f"  [{label}] includes {len(raw['bet365'])} bundled Bet365 matches")
    ...
    # ── Step 3-4: Init engine and index Bet365 ──
    ...
    # ── Step 4: Run inference per provider ──
    all_pushable: list = []
    provider_stats: dict = {}
    total_processed = 0
    ...
    for p in providers:
        ...
        if provider_b365_raw:
            ...
            print(f"\n  [{name}] Using bundled Bet365 pool: {len(b365_records)} matches")
        elif is_paginated and shared_b365_records:
            b365_records = shared_b365_records
            b365_source = "shared"
            print(f"\n  [{name}] Using shared Bet365 pool: {len(b365_records)} matches")
        else:
            print(f"\n  [{name}] No Bet365 data available — skipping.")
            provider_stats[name] = {"total": 0, "pushable": 0}
            continue

        if b365_source != current_b365_source:
            print(f"  Indexing {len(b365_records)} B365 matches (source: {b365_source})...")
    ...
    print("=" * 70)
    print(f"\n  Data fetched:")
    if shared_bet365_raw:
        print(f"    Bet365 (shared): {len(shared_bet365_raw)}")
    for name in provider_names:
        b365_bundled = len(fetch_data.get(f"{name}_bet365", []))
        if b365_bundled:
            print(f"    {name} Bet365 (bundled): {b365_bundled}")
    for name in provider_names:
        print(f"    {name} matches: {len(fetch_data.get(name, []))}")
